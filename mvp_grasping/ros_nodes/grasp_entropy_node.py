    ...
    def update_service_handler(self, req):
        ...
        with TimeIt('Total'):
            with TimeIt('Update Histogram'):
                ...
                # Do grasp prediction
                depth_crop, depth_nan_mask = process_depth_image(depth, self.img_crop_size, 300, return_mask=True)
                ...
                y = ((np.vstack((np.linspace((imh - self.img_crop_size) // 2, (imh - self.img_crop_size) // 2 + self.img_crop_size, depth_crop.shape[0], np.float), )*depth_crop.shape[1]).T - self.cam_K[1,2])/self.cam_K[1, 1] * depth_crop).flatten()
            ...
            with TimeIt('Response'):
                ...
                p.x = diff[0]
                p.y = diff[1]
                p.z = -1 * ((move_amt - np.linalg.norm(diff))/move_amt * 0.01 + 0.015)
                ret.viewpoint = p
